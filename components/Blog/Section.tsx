  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-8 overflow-hidden">
      ...
      <div className="bg-primary-2 pt-7 sm:pt-16 pb-28">
        ...
        >
          {props.loading &&
            [...new Array(15)].map((item: blogType, index: Number) => {
              return (
                <SwiperSlide key={`blog-${index}`} className=" !h-auto">
                  <BlogCard isSkeleton />
                </SwiperSlide>
              );
            })}
          ...
          <div className="flex items-center absolute left-1/2 -translate-x-1/2 gap-2 mt-10">
            ...
            >
              <FaChevronRight className="w-4 sm:w-6 h-4 sm:h-6" color="white" />
            </button>
            ...
            <button
              onClick={handleNext}
              className={` z-10 flex items-center mx-10 justify-center rounded-md bg-primary-1 w-6 sm:w-11 h-6 sm:h-11 ${
                sliderIndex === props.data.length - slidePerView
                  ? "opacity-50 cursor-default"
                  : ""
              }`}
            >
              <FaChevronLeft className="w-4 sm:w-6 h-4 sm:h-6" color="white" />
            </button>
          </div>
        </Swiper>
      </div>
